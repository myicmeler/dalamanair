      ...
      <div style={{maxWidth:'800px', margin:'0 auto', padding:'56px 20px'}}>
        ...
        {[
          ...
          <div key={s.num} style={{display:'flex', gap:'24px', marginBottom:'32px', alignItems:'flex-start'}}>
            ...
            <div style={{flex:1, borderBottom:'1px solid rgba(255,255,255,0.08)', paddingBottom:'32px'}}>
              <h3 style={{fontSize:'17px', fontWeight:'500', color:'#ffffff', marginBottom:'8px'}}>{s.title}</h3>
              <p style={{fontSize:'14px', color:'rgba(255,255,255,0.5)', lineHeight:'1.7', margin:0}}>{s.desc}</p>
            </div>
          </div>
        ))}
        ...
        <div style={{marginTop:'48px', backgroundColor:'#0f1419', borderRadius:'10px', padding:'32px', textAlign:'center'}}>
          ...
          <Link href="/" style={{display:'inline-block', backgroundColor:'#f4b942', color:'#ffffff', padding:'13px 28px', borderRadius:'4px', fontSize:'13px', fontWeight:'600', letterSpacing:'0.06em', textTransform:'uppercase', textDecoration:'none'}}>
            Search transfers →
          </Link>
        </div>
