    ...
    <div className="min-h-screen bg-ink text-paper">
      ...
      <div className="px-10 py-8 max-w-4xl">
        <div className="flex justify-between items-start mb-6">
          ...
          <button onClick={() => router.back()} className="text-sm text-muted hover:text-paper transition-colors">
            ← Edit search
          </button>
        </div>
          ...
          {(['price','rating','capacity'] as const).map(s => (
            <button key={s} onClick={() => setSort(s)}
              className={`text-xs px-3 py-1.5 rounded border transition-all capitalize ${
                sort === s ? 'border-paper/35 text-paper' : 'border-border text-muted hover:text-paper'
              }`}>{s}</button>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <div className="text-muted text-sm py-20 text-center">No vehicles available for {passengers} passengers.</div>
        ) : (
            ...
            {getSortedVehicles().map((v: any) => {
              ...
              return (
                <div key={v.id} onClick={() => handleSelect(v.id)}
                  className={`border rounded-lg px-6 py-5 grid grid-cols-[1fr_auto] gap-4 items-center cursor-pointer transition-all ${
                    isSelected ? 'border-paper/50 bg-white/[0.04]' : 'border-border hover:border-paper/25 hover:bg-white/[0.02]'
                  }`}>
                  <div>
                    <p className="text-xs tracking-widest text-muted uppercase mb-1">{v.type}</p>
                    ...
                    <div className="flex items-center gap-1.5">
                      <div className="flex gap-0.5">
                        {[1,2,3,4,5].map(i => (
                          <div key={i} className={`w-2.5 h-2.5 ${i <= Math.round(rating) ? 'opacity-80' : 'opacity-15'}`}
                            style={{ background: 'currentColor', clipPath: 'polygon(50% 0%,61% 35%,98% 35%,68% 57%,79% 91%,50% 70%,21% 91%,32% 57%,2% 35%,39% 35%)' }} />
                        ))}
                      </div>
                      <span className="text-xs text-muted">{rating.toFixed(1)} ({reviews} reviews)</span>
                    </div>
                    <p className="text-xs text-muted mt-1">{v.provider?.company_name}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-2xl font-medium">€ {price}</p>
                    {tripType === 'return' && <p className="text-xs text-muted mb-3">+ € {returnPrice} return</p>}
                    <button className={`text-xs px-4 py-2 rounded border transition-all ${
                      isSelected ? 'bg-paper text-ink border-paper' : 'border-border text-muted hover:border-paper/35 hover:text-paper'
