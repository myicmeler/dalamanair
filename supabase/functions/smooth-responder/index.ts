...
// New-format secret keys arrive as a JSON object keyed by name.
// The legacy service_role JWT is still read so the migration can happen in stages.
const SECRET_KEYS: Record<string,string> = (() => {
  try { return JSON.parse(Deno.env.get('SUPABASE_SECRET_KEYS') ?? '{}') } catch { return {} }
})()
const SECRET_KEY_VALUES = Object.values(SECRET_KEYS).filter(Boolean)
...
// Pickup times are stored exactly as the customer typed them and must never be
// converted. timeZone:'UTC' pins the render regardless of the runtime's locale.
function formatDate(dt: string) {
  return new Date(dt).toLocaleDateString('en-GB', { weekday:'short', day:'numeric', month:'short', timeZone:'UTC' })
}
...
serve(async (req) => {
  ...
  try {
    const supabase = createClient(Deno.env.get('SUPABASE_URL') ?? '', ADMIN_KEY)

    // --- AUTHORIZATION ---
    // The digest emails every approved provider, so it must only be triggered by
    // the scheduled cron or an admin. Three accepted callers:
    //   * a new-format secret key on the `apikey` header (cron / pg_net)
    //   * the legacy service_role JWT on `Authorization` (kept during migration)
    //   * a signed-in admin
    // Secret keys are NOT JWTs and are rejected on the Authorization header by
    // the platform, so pg_net must send them as `apikey`. Only values from
    // SUPABASE_SECRET_KEYS count — the publishable/anon key that supabase-js
    // puts on `apikey` for every browser request must never grant privilege.
    const apiKeyHeader = (req.headers.get('apikey') ?? '').trim()
    const bearer = (req.headers.get('Authorization') ?? '').replace(/^Bearer\s+/i, '').trim()

    const isSecretKeyCaller = !!apiKeyHeader && SECRET_KEY_VALUES.includes(apiKeyHeader)
    const isLegacyKeyCaller = !!bearer && !!LEGACY_SERVICE_KEY && bearer === LEGACY_SERVICE_KEY

    if (!isSecretKeyCaller && !isLegacyKeyCaller) {
      if (!bearer) {
