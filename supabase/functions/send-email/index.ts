...
function getTemplate(type: string, data: any): { subject: string, html: string } {
  switch (type) {
    ...
    case 'provider_confirmed_acknowledge': return { subject:`Provider confirmed — please acknowledge — ${data.pickup} → ${data.dropoff}`, html:wrap(card('Provider confirmed','Action needed — acknowledge the booking',`<p style="font-size:14px;color:rgba(255,255,255,0.6);line-height:1.7;margin:0 0 16px"><strong style="color:#fff">${data.providerName}</strong> has confirmed your booking. Please log in to acknowledge and fully confirm.</p>${rows([['Route',`${data.pickup} → ${data.dropoff}`],['Date',data.date],['Time',data.time],['Provider',data.providerName],['Price',`${sym(data.currency ?? 'EUR')} ${data.price}`]])}${timeNotice()}${btn('Acknowledge booking →',`${APP_URL}/bookings/`)}<p style="font-size:12px;color:rgba(255,255,255,0.3);margin:12px 0 0">Pay your driver directly on transfer day.</p>`)) }

    case 'driver_assigned': return { subject:`Your driver is assigned — ${data.pickup} → ${data.dropoff} · ${data.date}`, html:wrap(card('Driver assigned','Your driver is confirmed',`<p style="font-size:14px;color:rgba(255,255,255,0.6);line-height:1.7;margin:0 0 16px">${data.providerName ? `<strong style="color:#fff">${data.providerName}</strong> has assigned your driver.` : 'Your driver has been assigned.'} They will meet you at pickup.</p>${rows([['Route',`${data.pickup} → ${data.dropoff}`],['Date',data.date],['Time',data.time],['Driver',data.driverName],...(data.driverPhone?[['Driver phone',data.driverPhone] as [string,string]]:[]),...(data.vehicle?[['Vehicle',data.vehicle] as [string,string]]:[])])}${timeNotice()}${data.driverPhone?`<div style="background:rgba(244,185,66,0.08);border:1px solid rgba(244,185,66,0.2);border-radius:6px;padding:12px 16px;margin:0 0 16px"><p style="font-size:12px;color:rgba(255,255,255,0.6);margin:0">Need to reach your driver? Call or WhatsApp <strong style="color:#fff">${data.driverPhone}</strong> on the day.</p></div>`:''}${btn('View booking →',`${APP_URL}/bookings/`)}<p style="font-size:12px;color:rgba(255,255,255,0.3);margin:12px 0 0">Pay your driver directly on transfer day.</p>`)) }
